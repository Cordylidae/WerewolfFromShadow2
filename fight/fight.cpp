...
int turns,z=1;
bool arrow=false,att=false,kkd=true,vuborp=false,vubore=false,onoff=false;
...
int main()
{
	...
	gf_<<gf;
	Music music;
	music.openFromFile(gf_.str()+".ogg");
	music.setLoop(true);
	...
	Texture textt;
	textt.loadFromFile("ff/options.png");
	Sprite s;
	s.setTexture(textt);
	
	Texture buttons2_;
	buttons2_.loadFromFile("ff/buttons2.png");
	Sprite buttons2;
	buttons2.setTexture(buttons2_);
	...
	Texture help1;
	help1.loadFromFile("ff/help.png");
	Sprite help;
	help.setTexture(help1);

	Texture tt1;
	tt1.loadFromFile("ff/tutorial.png");
	Sprite tt;
	tt.setTexture(tt1);
	//////////////////////////////////////
    ...
    while (window.isOpen())
    {
		...
		if((zz==endP || zz==attackESw || zz==attackES || zz==attackEB) && !victory)
		{
			...
			if(zz==endP){turns=DL::choose(player.getMana(),evilplayer.getMana(),player.getHelth(),evilplayer.getHelth(),player.getArmor(),evilplayer.getArmor(),x,y,x1,y1,evilplayer.getSurikens(),prop,spell.icesuriken,spell.nomagic);
			...
			if(turns==1)
			{
				...
				int a=0,b=0;
				DL::turn(x,y,x1,y1,a,b,spell.icesuriken);
		...
		if(pozition.x>=100 && pozition.x<=100+60 && pozition.y>=500 && pozition.y<=500+80 && !tut && Mouse::isButtonPressed(Mouse::Left) && (zz==endE || zz==startP || zz==endP || zz==startE))window.close();
		if(pozition.x>=40 && pozition.x<=40+70 && pozition.y>=565 && pozition.y<=565+50 && !tut && Mouse::isButtonPressed(Mouse::Left) && (zz==endE || zz==startP || zz==endP || zz==startE)){kkd=true;}//40 565 70 50
		if(kkd){help.setPosition(0,0);window.draw(help);if(Keyboard::isKeyPressed(Keyboard::Escape))kkd=false;}
		if(pozition.x>=20 && pozition.x<=20+75 && pozition.y>=505 && pozition.y<=505+55 && !tut && Mouse::isButtonPressed(Mouse::Left)){aa=option;}
		if(tut){tt.setPosition(0,0);window.draw(tt);}
		window.display();
	...
	case option:
		{
			  ...
			  while (window.pollEvent(event))
			{
				...
				if(event.type == Event::MouseButtonPressed)
				{
					if(event.key.code == Mouse::Button::Left && pozition.x>=116 && pozition.x<=116+40 && pozition.y>=238 && pozition.y<=238+40){vuborp=true;player.swap(vuborp);}
					if(event.key.code == Mouse::Button::Left && pozition.x>=235 && pozition.x<=235+40 && pozition.y>=238 && pozition.y<=238+40){vuborp=false;player.swap(vuborp);}
					if(event.key.code == Mouse::Button::Left && pozition.x>=113 && pozition.x<=113+40 && pozition.y>=495 && pozition.y<=495+40){vubore=true;evilplayer.swap(vubore);}
					if(event.key.code == Mouse::Button::Left && pozition.x>=228 && pozition.x<=228+40 && pozition.y>=495 && pozition.y<=495+40){vubore=false;evilplayer.swap(vubore);}
					if(event.key.code == Mouse::Button::Left && pozition.x>=374 && pozition.x<=374+40 && pozition.y>=316 && pozition.y<=316+40){onoff=true;music.play();}
					if(event.key.code == Mouse::Button::Left && pozition.x>=530 && pozition.x<=530+40 && pozition.y>=316 && pozition.y<=316+40){onoff=false;music.pause();}
					if(event.key.code == Mouse::Button::Left && pozition.x>=372 && pozition.x<=372+40 && pozition.y>=110 && pozition.y<=110+40 && music.getVolume()>=10){music.setVolume(music.getVolume()-10);}
					if(event.key.code == Mouse::Button::Left && pozition.x>=535 && pozition.x<=535+40 && pozition.y>=110 && pozition.y<=110+40 && music.getVolume()<=90){music.setVolume(music.getVolume()+10);}//372 110 40 40
				}
				if(event.type == Event::KeyPressed)
				{
					if(event.key.code == Keyboard::Escape)aa=game;
				}
			}
			 window.clear();
			...
			if(music.getVolume()<10)music.setVolume(10);

			 if(vuborp){
				 buttons2.setTextureRect(IntRect(40,0,40,40));
				 buttons2.setPosition(116,238);
				 window.draw(buttons2);
				 buttons2.setTextureRect(IntRect(0,0,40,40));
				 buttons2.setPosition(235,238);
				 window.draw(buttons2);
			 }
			 else{
				 buttons2.setTextureRect(IntRect(40,0,40,40));
			...
			if(vubore){
				 buttons2.setTextureRect(IntRect(40,0,40,40));
				 buttons2.setPosition(113,495);
				 window.draw(buttons2);
				 buttons2.setTextureRect(IntRect(0,0,40,40));
				 buttons2.setPosition(228,495);
				 window.draw(buttons2);
			 }
			 else{
				 buttons2.setTextureRect(IntRect(40,0,40,40));
			 ...
			 if(onoff){
				 buttons2.setTextureRect(IntRect(40,0,40,40));
				 buttons2.setPosition(374,316);
				 window.draw(buttons2);
				 buttons2.setTextureRect(IntRect(0,0,40,40));
				 buttons2.setPosition(530,316);
				 window.draw(buttons2);
			 }
			 else{
				 buttons2.setTextureRect(IntRect(40,0,40,40));
		...
		ostringstream volume_;

		
		text2.setColor(Color::Black);
		volume_ << music.getVolume();
		text2.setString(volume_.str()+" %");
		text2.setPosition(445,105);
		window.draw(text2);
